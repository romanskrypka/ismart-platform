...
/*
  ...
  Запуск:
    node scripts/validate-json.js [dir1] [dir2] ...

  Пример:
    node scripts/validate-json.js data/json
*/
...
function main() {
  ...
  if (roots.length === 0) {
    console.error('Укажите директорию(и) для проверки. Например: node scripts/validate-json.js data/json');
    process.exit(2);
  }
...
// #!/bin/sh
// node scripts/validate-json.js data/json || exit 1
