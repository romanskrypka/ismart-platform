...
> **Смотрите файл `observability_guide.md`** — полное руководство по наблюдаемости:
> 1. **Философия «Прозрачной коробки»** — почему нужно видеть каждый шаг
> 2. **Паттерн Pipeline Tap** — точки съёма данных на каждом этапе конвейера
> 3. **Уровень 1: Django** — Middleware, декоратор `@log_pipeline_step`, структурированное логирование
> 4. **Уровень 2: ИИ-пайплайны** — Langfuse `@observe` для RAG, LLM-вызовов, агентов
> 5. **Уровень 3: Мультиагентные системы** — трассировка траекторий рассуждений
> 6. **Промпты для Cursor** — готовые шаблоны для внедрения наблюдаемости
> 7. **Правило для `.cursorrules`** — автоматическое применение наблюдаемости ко всему новому коду
